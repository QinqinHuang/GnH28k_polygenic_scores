...
#  (2) select the best C+T PGS
#  (3) calculate boostrapping 95% CIs for the best PGS
#-------------------------------------------------------
...
for(ii in 1:nrow(traitGWAS)) {
  ...
  binaryflag = ifelse(traitGWAS$traittype[ii]=="binary", yes=T, no=F)
...
length(filelist) == nrow(traitGWAS)
